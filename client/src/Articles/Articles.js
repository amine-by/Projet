...
export default function Articles() {
  ...
  const refreshRows = () => {
    setOpen(false);
    setRows([]);
    axios.post("/articles/recherche", {}).then((resultat) => {
      resultat.data.map((r) => {
        return (r.image =
  ...
  useEffect(() => {
    ...
    axios.post("/marques/recherche", {}).then((resultat) => {
      resultat.data.forEach((d) =>
    ...
    <div>
      ...
      >
        <DialogTitle id="form-dialog-title">
          {id === "" ? <div>Ajouter Article</div> : <div>Modifier Article</div>}
        </DialogTitle>
        <DialogContent>
          <TextField
            type="file"
